...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
    {
        ...
        //window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y), check_bb.Max, GetColorU32(ImVec4(0.34f, 1.0f, 0.54f, 1.0f)), 0);
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 10, check_bb.Min.y + 5), ImVec2(check_bb.Min.x + 20, check_bb.Min.y + 15), ImColor(255, 0, 0));
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Min.x + 27, check_bb.Min.y + 2), label);
    return pressed;
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Max.x, frame_bb.Max.y + 5), ImColor(0.19f, 0.18f, 0.21f, 1.00f), 4.f);


    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(ImVec2(frame_bb.Min.x + 5, frame_bb.Min.y + 5), value_bb.Max, preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));

    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Max.x + 12, frame_bb.Min.y + style.FramePadding.y + 1), label);
